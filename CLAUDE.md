...
- The Sleep Data screen parses `/sleep_stages.csv` on entry (`sleepSummaryDirty`) into totals + a run-length-encoded two-lane hypnogram; reboot-safe since it reads from SD. When no `/sleep_stages.csv` exists it renders an empty-state hypnogram from a fixed table (`fillPreviewSummary()`) so the screen is never blank on a fresh boot; the first tracked session replaces it. Note the figures it shows (23:15-06:12, 6h57m / 4h43m light / 2h14m deep) are that placeholder, not recorded data.
